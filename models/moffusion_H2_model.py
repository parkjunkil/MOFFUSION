...
class MOFFUSIONH2Model(BaseModel):
    def name(self):
        return 'MOFFUSION-H2-Model'
    ...
    def set_input(self, input=None, max_sample=None):
        ...
        self.x = input['sdf']
        self.H2 = input['H2']
        #self.uc_H2 = torch.full(self.H2.shape, -1).to(self.device)

        if max_sample is not None:
            self.x = self.x[:max_sample]
            self.H2 = self.H2[:max_sample]
            #self.uc_H2 = self.uc_H2[:max_sample]
    ...
    def forward(self):
        ...
        c = self.H2.view(B, 1, 1, 1, 1).repeat(1, self.unet_params.out_channels, self.unet_params.image_size, self.unet_params.image_size, self.unet_params.image_size).float()
    ...
    @torch.no_grad()
    def inference(self, data, ddim_steps=None, ddim_eta=0., infer_all=False, max_sample=16):
    ...
    @torch.no_grad()
    def cond(self, ngen=1, target=0.4, ddim_steps=200, ddim_eta=0.):
        ddim_sampler = DDIMSampler(self)
        ...
        c = target.repeat(B, self.unet_params.out_channels, self.unet_params.image_size, self.unet_params.image_size, self.unet_params.image_size).float()
        c = c.to(self.device)
        samples, intermediates = ddim_sampler.sample(S=ddim_steps,
                                                     batch_size=B,
                                                     shape=shape,
