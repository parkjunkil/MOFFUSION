...
class MOFFUSIONTopoModel(BaseModel):
    def name(self):
        return 'MOFFUSION-Topo-Model'
    ...
    def set_input(self, input=None, max_sample=None):
        ...
        if max_sample is not None:
            ...
            self.node1 = self.node1[:max_sample]
            self.node2 = self.node2[:max_sample]
            self.edge = self.edge[:max_sample]

            #self.di = self.di[:max_sample]
            #self.uc_di = self.uc_di[:max_sample]

        vars_list = ['x', 'topo', 'node1', 'node2', 'edge']
    ...
    @torch.no_grad()
    def cond(self, ngen=1, target='cds', ddim_steps=200, ddim_eta=0.):
        ddim_sampler = DDIMSampler(self)
        ...
        B = ngen
        shape = self.z_shape
        target = torch.LongTensor(self.encoders['topo'].transform([target]))
