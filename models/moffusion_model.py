# Reference: The model architecture is modified from the repo: https://https://github.com/yccyenchicheng/SDFusion
...
class MOFFUSIONModel(BaseModel):
    def name(self):
        return 'MOFFUSION-Model'
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            '''
            #Note!!
            For the scheduler, both StepLR and ReduceLROnPlateau can be utilized.
            In the case of VQ-VAE and unconditional moffusion, StepLR is used, while ReduceLROnPlateau is used for MOF-Constructor and the other conditional models.
            No significant difference in performance was observed between the two schedulers.
    ...
    def shape_comp(self, shape, xyz_dict, ngen=1, ddim_steps=100, ddim_eta=0.0):        
        from utils.demo_util import get_partial_shape_MOF
        ddim_sampler = DDIMSampler(self)
        ...
        # get partial shape
        ret = get_partial_shape_MOF(shape, xyz_dict=xyz_dict, z=z)
