...
class MOFFUSIONVfSaModel(BaseModel):
    def name(self):
        return 'MOFFUSION-VF-SA-Model'
    ...
    def set_input(self, input=None, max_sample=None):
        ...
        self.sa = input['sa']
        self.vf = input['vf']
        #self.uc_di = torch.full(self.di.shape, -1).to(self.device)


        if max_sample is not None:
            self.x = self.x[:max_sample]
            self.sa = self.sa[:max_sample]
            self.vf = self.vf[:max_sample]
    ...
    def forward(self):
        ...
        c1 = self.vf.view(B, 1, 1, 1, 1).repeat(1, self.unet_params.out_channels, self.unet_params.image_size, self.unet_params.image_size, self.unet_params.image_size).float()

        c2 = self.sa.view(B, 1, 1, 1, 1).repeat(1, self.unet_params.out_channels, self.unet_params.image_size, self.unet_params.image_size, self.unet_params.image_size).float()
        ...
        z_noisy, target, loss, loss_dict = self.p_losses(z, c1, c2, t)
    ...
    @torch.no_grad()
    def cond(self, ngen=1, target1=0.5, target2=0.5, ddim_steps=200, ddim_eta=0.):
        ddim_sampler = DDIMSampler(self)
        ...
        c1 = target1.repeat(B, self.unet_params.out_channels, self.unet_params.image_size, self.unet_params.image_size, self.unet_params.image_size).float()
